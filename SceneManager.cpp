...
void SceneManager::LoadScene( char *name, char *path )
{
	...
	// シーンスクリプトは不要になったため、破棄します。
	SAFE_DELETE( script );

	// ビュー錐台がシーンに合うように、新しい投影行列を設定します。
	...
	for( unsigned long m = 0; m < m_mesh->GetStaticMesh()->NumMaterials; m++ )
	{
		...
		// マテリアルが見つからず、面を無視するように設定されていない場合は、マテリアルを追加します。
	...
	{
		// このフレームにオクルーダーが含まれているかどうかを確認してください。
		if( strncmp( "oc_", frames->GetCurrent()->Name, 3 ) == 0 )
		{
			// あるの場合は、オクルーダーをロードして、次のフレームに進みます。
		...
		// このフレームがスポーンポイントであるかどうかを確認してください。
		if( strncmp( "sp_", frames->GetCurrent()->Name, 3 ) == 0 )
		{
			// このスポーンポイントでのスポナーオブジェクトの実際の名前を取得します。
			...
			// プレイヤーのスポーンポイントかどうかを確認します。
			if( stricmp( name, "player.txt" ) == 0 )
			{
				// プレイヤーのスポーンポイントの半径フレームの名前を取得します。
			...
			// スポナーの名前を作成するために使用された文字列バッファーを破棄します。
...
//-----------------------------------------------------------------------------
// 指定されたID（リンクリスト内の位置）のスポーンポイントを返します。
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// 指定されたスポーンポイントのID（リンクリスト内の位置）を返します。
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// シーンとそのすべてのオブジェクトとの光線の交差の結果を返します。
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// 指定されたオクルーダーのオクルージョンボリュームを作成します。
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// 指定されたオブジェクトに対してシーンの葉を再帰的にチェックします。
//-----------------------------------------------------------------------------
