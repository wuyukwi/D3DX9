/********************************************************************************* 
  ...
  *FileName: AllocateHierarchy.cpp  
            コウ  キガク
  *Author:  Huang QiYue
  *Version:  1.0
  *Date:  2022/01/28
...
HRESULT CAllocateHierarchy::GenerateSkinnedMesh(IDirect3DDevice9* pd3dDevice, D3DXMESHCONTAINER_DERIVED* pMeshContainer)
{
	...
	// 指定されたインデックスバッファーを持つ三角形メッシュ最大影響を取得
	...
	// デバイス能力とNumMaxFaceInflにより, スキンメッシュのボーンの上限値を設定
...
HRESULT CAllocateHierarchy::CreateFrame(LPCSTR Name, LPD3DXFRAME* ppNewFrame)
{
	...
	*ppNewFrame = NULL;
	...
	pFrame = new D3DXFRAME_DERIVED;  // フレームオブジェクト作成
	if (pFrame == NULL)
	{
		return E_OUTOFMEMORY;
	}
...
//-----------------------------------------------------------------------------
// Desc: メンバー関数 GenerateSkinnedMesh(pMeshContainer);
//       メッシュオブジェクト作成、データを保存
//-----------------------------------------------------------------------------
	...
	LPD3DXMESHCONTAINER* ppNewMeshContainer)
{
	...
	UINT NumFaces;       //
	UINT iMaterial;      // ループ用変数
	UINT iBone,cBones;         // ボーンの数量
	...
	*ppNewMeshContainer = NULL;
	...
	if( NumMaterials > 0 )
	{
		...
		for( iMaterial = 0; iMaterial < NumMaterials; iMaterial++ )
		{
			...
			{
				if (FAILED(D3DXCreateTextureFromFile(pd3dDevice, (LPCWSTR)pMeshContainer->pMaterials[iMaterial].pTextureFilename,
